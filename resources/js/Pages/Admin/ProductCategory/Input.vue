...
const isModeCreate = computed(() => {
    ...
    return (! props.productCategory.id);
...
onMounted(() => {
    ...
    if (props.productCategory.id) {
        ...
        params.value = props.productCategory;
...
const onSubmit = () => {
    ...
    if(isModeEdit.value) {
        ...
        url = route('admin.product_category.update', {product_category: props.productCategory.id});
        data = {
            _method: 'PUT',
            ...params.value,
        };
...
const onCancel = () => {
    ...
    router.visit(route('admin.product_category.index'));
...
<template>
    <Head title="カテゴリ（360°VR）追加・編集" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-indigo-50 leading-tight">カテゴリ（360°VR）追加・編集</h2>
        </template>
        ...
        <FooterButtons
            ...
            <template v-slot:left>
                <SubButton size="md" :href="route('admin.product_category.input')" @click="onCancel">新規追加</SubButton>
            </template>
        </FooterButtons>
