...
onMounted(() => {
    ...
    if (props.shop.id) {
        ...
        params.value.name = props.shop.name;
        params.value.email = props.shop.email;
...
const onSubmit = () => {
    ...
    if(isModeEdit.value) {
        ...
        url = route('admin.shop.update', {shop: props.shop.id});
        data = {
            _method: 'PUT',
            ...params.value,
        };
...
const onCancel = () => {
    ...
    router.visit(route('admin.shop.index'));
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-indigo-50 leading-tight">店舗追加・編集</h2>
        </template>
        ...
        <FooterButtons
            ...
            <template v-slot:left>
                <SubButton size="md" :href="route('admin.shop.input')" @click="onCancel">新規追加</SubButton>
            </template>
        </FooterButtons>
