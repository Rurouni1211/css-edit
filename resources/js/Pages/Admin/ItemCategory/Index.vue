...
const getItemCategories = () => {
    ...
    itemCategoryLinks.value = {};

    const page = currentPage.value;
    const url = route('admin.item_category.list', {page: page});
    const data = {
        params: {
    ...
    axios.get(url, data)
        .then((response) => {
            ...
            itemCategoryLinks.value = response.data.meta.links;
...
const onDelete = (itemCategory) => {

    const url = route('admin.item_category.destroy', itemCategory.id);
...
<template>
    <Head title="カテゴリ管理（完成品）" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-indigo-50 leading-tight">カテゴリ管理<small>（完成品）</small></h2>
        </template>

        <div class="py-3 ms:py-6">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="text-right mb-3">
                    <NormalButton size="xs" :href="route('admin.item_category.input')">新規登録</NormalButton>
                </div>
                    ...
                    <div class="p-5 sm:p-10 text-gray-900">
                        ...
                        <ResponsiveTable class="mb-4">
                            <table class="bg-white w-full text-sm">
                                ...
                                <tbody>
                                <template v-for="category in itemCategories" :key="category.id">
                                    <tr class="odd:bg-gray-50 hover:bg-indigo-100">
                                        ...
                                        <td class="border border-gray-300 p-2 text-nowrap">
                                            <NormalButton size="xs" class="mr-2" :href="route('admin.item_category.input', category.id)">編集</NormalButton>
                        ...
                        <Pagination :links="itemCategoryLinks" @move-page="onMovePage" />
                    </div>
                </div>
            </div>
        </div>
