...
const formUrl = route(`${multiAuthGuard}.profile.update`);
const form = useForm({
    name: user.name,
    email: user.email,
});
...
<template>
    <section>
        ...
        <form @submit.prevent="form.patch(formUrl)" class="mt-6 space-y-6">
            <div>
            ...
            <div class="flex items-center gap-4">
                ...
                >
                    <p v-if="form.recentlySuccessful" class="text-sm text-green-600">保存しました</p>
                </Transition>
            </div>
        </form>
    </section>
